            ...
            for sym, c in custodies.items():
                if c["custodyAccount"] == target_addr and sym == symbol:
                    ...
                    return {
                        ...
                        "target_oracle": next((c.get("intOracleAccount", c.get("oracleAddress", "")) for s, c in custodies.items() if s == sym), ""),
                        "collateral_oracle": next((c.get("intOracleAccount", c.get("oracleAddress", "")) for s, c in custodies.items() if s == collateral_sym), ""),
